...
const SWE_ROW_WITH_SCORES = /SWE-Bench Verified 80.8 80.6/;
const SWE_ROW_WITH_DASH = /SWE-Bench Verified 80.8 -/;
...
describe("BenchmarkTable", () => {
  it("renders benchmark rows and selected model columns", () => {
    ...
    expect(
      screen.getByRole("columnheader", { name: "Benchmark" })
    ).toBeVisible();
    expect(
  ...
  it("shows an empty state when no models are selected", () => {
    render(
      <BenchmarkTable rows={buildBenchmarkRows([])} selectedModels={[]} />
    );

    expect(screen.getByRole("heading", { name: "Benchmarks" })).toBeVisible();
    expect(
      screen.getByText("Select at least one model to compare.")
    ).toBeVisible();
  });

  it("preserves selected model order in column headers", () => {
    const selectedModels = ["gemini-3-1-pro", "claude-opus-4-6"].map(
      (modelId) => {
        const model = modelSummaries.find(
